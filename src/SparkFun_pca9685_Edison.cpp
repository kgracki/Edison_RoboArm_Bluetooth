  ...
  _minAngle(0), _maxAngle(180)
{
  ...
  _pca_port->writeReg(MODE1, AI | ALLCALL); //enable autoincrement and osc
  _pca_port->writeReg(MODE2, OUTDRV); // open drain outs
/*
   uint8_t modeOUT = _pca_port->readReg(MODE2);
  modeOUT &= ~OUTDRV;
  _pca_port->writeReg(MODE2, modeOUT);
  */
}
...
uint16_t pca9685::readChl0RegON()
{
	uint16_t chl0RegON = _pca_port->readReg(LED0_ON_L);
	chl0RegON |= _pca_port->readReg(LED0_ON_H)<<8;

	return chl0RegON;
}

uint16_t pca9685::readChl0RegOFF()
{
	uint16_t chl0RegOFF = _pca_port->readReg(LED0_OFF_L);
	chl0RegOFF |= _pca_port->readReg(LED0_OFF_H)<<8;
...
{
  uint16_t modeReg = _pca_port->readReg(MODE2);
